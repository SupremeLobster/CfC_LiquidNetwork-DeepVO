...
class DeepVO_CfC(nn.Module):
    def __init__(self, imsize1, imsize2, batchNorm=True):
        super(DeepVO_CfC,self).__init__()
        # CNN
        ...
        self.rnn = Cfc(in_features=int(np.prod(__tmp.size())),
                      ...
                      out_feature=6,  # Hardcoded on purpose
                      return_sequences=True,  # Necessary to output the correct shape
                      hparams=CFC,
                      use_mixed=CFC["use_mixed"])
    ...
    def forward(self, x, times): 
        ...
        # CfC exclusive as time appears as an explicit term in the neuron's equations
