...
#         print("Poses: ", poses)
#         fpaths = glob.glob('{}{}\\image_0\\*.png'.format(par.image_dir, folder))
        fpaths = [f'{par.image_dir}{folder}\\image_0\\{image_file}' for image_file in os.listdir('{}{}\\image_0\\'.format(par.image_dir, folder)) if image_file.endswith(".png")]
        times = []
        with open(f'{par.image_dir}{folder}\\times.txt') as f:
...
#         print("Fpaths: ", fpaths)
        ...
        if seq_len_range[0] == seq_len_range[1]:
            ...
            for st in start_frames:
                ...
                x_times = [times[i:i+seq_len] for i in range(st, n_frames, jump)]
            ...
            for i in range(sample_times):
                ...
                while True:
                    ...
                    if start + n < n_frames:
                        ...
                        X_path.append(x_seg)
                        X_time.append(times[start:start+n])
                        if not pad_y:
    ...
    data = {'seq_len': X_len, 'image_path': X_path, 'time_stamps': X_time, 'pose': Y}
    df = pd.DataFrame(data, columns = ['seq_len', 'image_path', 'time_stamps', 'pose'])
    # print(df.time_stamps)
    ...
    if os.path.isfile('datainfo/df_train.csv'):
        df.to_csv('datainfo/df_val.csv', index=False)
    else:
        df.to_csv('datainfo/df_train.csv', index=False)
    return df
...
#             fpaths = glob.glob('{}{}/*.png'.format(par.image_dir, folder))
            fpaths = [image_file for image_file in os.listdir('{}{}\\image_0\\'.format(par.image_dir, folder)) if image_file.endswith(".png")]
            fpaths.sort()
...
class SortedRandomBatchSampler(Sampler):
    ...
    def __iter__(self):
        ...
        for v in self.unique_seq_lens:
            ...
            start_idx += n_sample
        # print("List batch indexes: ", list_batch_indexes)
...
class ImageSequenceDataset(Dataset):
    def __init__(self, info_dataframe, resize_mode='crop', new_sizeize=None, img_mean=None, img_std=1, minus_point_5=False):
        # Transforms
        transform_ops = []
        if resize_mode == 'crop':
        ...
        self.time_arr = np.asarray(self.data_info.time_stamps)
#         print("Image_Arr: ", self.image_arr)
    ...
    def __getitem__(self, index):
        ...
        time_stamp_sequence = torch.FloatTensor(np.asarray(self.time_arr[index]).astype(np.float64))
        # print("Dataset Index: ", index)
        # print("Time sequence: ", time_stamp_sequence)
        
        image_sequence = []
        for i, img_path in enumerate(image_path_sequence):
            # print(i)
        ...
        # print("Groundtruth sequence shape: ", groundtruth_sequence.shape)
        return sequence_len, image_sequence, time_stamp_sequence, groundtruth_sequence
